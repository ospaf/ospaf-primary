...
def generate_by_month (db, month_start):
    month_int = month_start/100
    saved_res = db["research_result"].find_one({"type": "monthly_repo_amount", "month": month_int})
    need_update = 0
    if saved_res:
        need_update = 1
    ...
    query_created = {"created_at_int": {"$gte": month_start, "$lt": month_end}}

    count = db["repositories"].find(query_created).count()
    if need_update:
        db["research_result"].update({"type": "monthly_repo_amount", "month": month_int}, {"$set": {"count": count}})
    else:
        db["research_result"].insert({"type": "monthly_repo_amount", "month": month_int, "count": count})

#begin at 2007-10, end at 2015-01
def generate_months(db):
...
def report_by_month (db, month_start):
    month_int = month_start/100
    saved_res = db["research_result"].find_one({"type": "monthly_repo_amount", "month": month_int})
    print str(month_int) + "\t" + saved_res["count"]
